...
std::shared_ptr<ChBody> AddGround(ChSystem& sys) {
	...
	auto fixedBody=chrono_types::make_shared<ChBodyEasyBox>(170, 10, 170, 2.4e-9,
								true, true, fixed_mat);    
...
std::shared_ptr<ChBody> AddLeftWall(ChSystem& sys) {
	...
	auto fixedBody=chrono_types::make_shared<ChBodyEasyBox>(10, 170, 170, 2.4e-9,
								true, true, fixed_mat);    
...
std::shared_ptr<ChBody> AddFrontWall(ChSystem& sys) {
	...
	auto fixedBody=chrono_types::make_shared<ChBodyEasyBox>(150, 170, 10, 2.4e-9,
								true, true, fixed_mat);    
...
std::shared_ptr<ChBody> AddTopWall(ChSystem& sys) {
	...
	auto fixedBody=chrono_types::make_shared<ChBodyEasyBox>(150, 10, 150, 2.4e-9,
								true, true, fixed_mat);    
    //fixedBody->SetBodyFixed(true);
...
int main(int argc, char* argv[]) {
	...
	//
	//
	auto constr_bottomPlate = chrono_types::make_shared<ChLinkMateGeneric>(true, true,
									       true, true,
									       true, true);
    constr_bottomPlate->Initialize(bottomPlate, mtruss, false, bottomPlate->GetFrame_REF_to_abs(),
				   bottomPlate->GetFrame_REF_to_abs()); 
    sys.Add(constr_bottomPlate);
	//
	auto constr_leftPlate = chrono_types::make_shared<ChLinkMateGeneric>(true, true,
									     true, true,
									     true, true);
    constr_leftPlate->Initialize(leftPlate, mtruss, false, leftPlate->GetFrame_REF_to_abs(),
				 leftPlate->GetFrame_REF_to_abs()); 
    sys.Add(constr_leftPlate);
	//
	auto constr_rightPlate = chrono_types::make_shared<ChLinkMateGeneric>(true, true,
									      true, true,
									      true, true);
    constr_rightPlate->Initialize(rightPlate, mtruss, false, rightPlate->GetFrame_REF_to_abs(),
				  rightPlate->GetFrame_REF_to_abs()); 
    sys.Add(constr_rightPlate);
	//
	auto constr_frontPlate = chrono_types::make_shared<ChLinkMateGeneric>(true, true,
									      true, true,
									      true, true);
    constr_frontPlate->Initialize(frontPlate, mtruss, false, frontPlate->GetFrame_REF_to_abs(),
				  frontPlate->GetFrame_REF_to_abs()); 
    sys.Add(constr_frontPlate);
	//
	auto constr_backPlate = chrono_types::make_shared<ChLinkMateGeneric>(true, true,
									     true, true,
									     true, true);
    constr_backPlate->Initialize(backPlate, mtruss, false, backPlate->GetFrame_REF_to_abs(),
				 backPlate->GetFrame_REF_to_abs()); 
	...
	auto materialFCM=chrono_types::make_shared<ChMaterialFCM>(ENm, ENa, mortar_layer,
								  alpha, beta, np, sgmTmax,
								  sgmTau0, kappa0, eta_inf);
    ...
    emitter_positions->Outlet() = ChCoordsys<>(ChVector<>(0, 200, 0),
					       Q_from_AngAxis(CH_C_PI_2, VECT_X));
    // center and alignment of the outlet
    ...
    mcreator_spheres->
      SetDiameterDistribution(chrono_types::make_shared<ChConcreteDistribution>(minD, maxD, cement,
										AtoC, WtoC,
										mortar_layer));
    mcreator_spheres->
      SetDensityDistribution(chrono_types::make_shared<ChConstantDistribution>(rho));	
	...
	if (std::fmod(step_num, 400) ==0) {
	  ...
	  for(int i=0; i<5; i++){
	    ...
	    auto pos = body->GetFrame_COG_to_abs().GetPos();
            fprintf(fptr, " %10.6f %10.6f  %10.6f\t ", cont_force.x(), cont_force.y(),
		    cont_force.z() );
	...
	if (std::fmod(step_num, 400) ==0) {
	  ...
	  for(int i=0; i<5; i++){
	    ...
	    fprintf(fptr, " %10.6f %10.6f  %10.6f\t ", pos.x(), pos.y(), pos.z() );
	    auto applForce=const_list[i]->Get_react_force();
	    fprintf(fptr, " %10.6f %10.6f  %10.6f\t ", applForce.x(), applForce.y(),
		    applForce.z() );  
	    }
	...
	//
	auto constr_topPlate = chrono_types::make_shared<ChLinkMateGeneric>(true, false,
									    true, true,
									    true, true);
    constr_topPlate->Initialize(topPlate, mtruss, false, topPlate->GetFrame_REF_to_abs(),
				topPlate->GetFrame_REF_to_abs()); 
    sys.Add(constr_topPlate);
    const_list.push_back(constr_topPlate);
	//
    auto body_force = chrono_types::make_shared<ChLoadBodyForce>(topPlate, ChVector<>(0, 5.2974, 0),
    ...
    motorY->Initialize(topPlate,               // slave body
                       mtruss,                // master body
                       ChFrame<>(topPlate->GetPos(), Q_from_AngZ(CH_C_PI_2) )
